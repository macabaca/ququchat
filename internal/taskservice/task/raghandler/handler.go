...
const defaultMaxMessagesPerSeg = 30
...
func (h *Handler) ExecuteRAG(ctx context.Context, payload *tasksvc.RAGPayload) (tasksvc.Result, error) {
	...
	if len(segments) == 0 {
		...
		if skippedTooLongMessages > 0 {
			final = fmt.Sprintf("%s，跳过超长消息 %d 条", final, skippedTooLongMessages)
		}
		return tasksvc.Result{
			Text:  &final,
			Final: &final,
			Payload: map[string]interface{}{
				...
				"skipped_too_long_messages": skippedTooLongMessages,
