...
func (l *RateLimiter) Wait(ctx context.Context, inputs []string) error {
	if l == nil || (l.rpm <= 0 && l.tpm <= 0) {
		return nil
	}
	reserveTokens := l.EstimateTokens(inputs)
	for {
...
func (l *RateLimiter) EstimateTokens(inputs []string) int {
	const baseOverheadTokens = 32
	total := baseOverheadTokens
	for _, input := range inputs {
		total += utf8.RuneCountInString(strings.TrimSpace(input))
	}
	if total < baseOverheadTokens {
		total = baseOverheadTokens
	}
	if l != nil && l.tpm > 0 && total > l.tpm {
		return l.tpm
	}
	return total
}
