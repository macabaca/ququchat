...
- `POST /api/users/me/avatar` 上传/更新当前用户头像（需要 `Authorization: Bearer <accessToken>`）
- `GET /api/users/:user_id/avatar/url` 获取用户头像下载链接（需要 `Authorization: Bearer <accessToken>`）
- `GET /api/users/:user_id/avatar/thumb/url` 获取用户头像缩略图下载链接（需要 `Authorization: Bearer <accessToken>`）
...
- 成功响应：`201 Created`
  ```json
  {
    "user": {"id": "uuid", "username": "alice", "status": "active", "avatar_attachment_id": null}
  }
  ```
- 失败响应：
...
## 上传/更新头像
- 方法与路径：`POST /api/users/me/avatar`
- 认证：需要在请求头携带 `Authorization: Bearer <accessToken>`
...
- 成功响应：`201 Created`
  ```json
  {
    ...
    "attachment": {
      ...
      "storage_key": "path/in/bucket/avatar.jpg",
...
- 失败响应：
  ...
  - `400` 文件为空 → `{"error":"文件为空"}`
  - `413` 文件过大 → `{"error":"文件过大"}`
  - `400` 只允许上传图片 → `{"error":"只允许上传图片"}`
  - `503` 对象存储未就绪 → `{"error":"对象存储未就绪"}`
  - `503` 对象存储配置缺失 → `{"error":"对象存储配置缺失"}`
  - `404` 用户不存在 → `{"error":"用户不存在"}`
  - `500` 更新头像失败 → `{"error":"更新头像失败"}`
  - `500` 上传头像失败 → `{"error":"上传头像失败"}`

## 获取头像下载链接
- 方法与路径：`GET /api/users/:user_id/avatar/url`
- 认证：需要在请求头携带 `Authorization: Bearer <accessToken>`
- 成功响应：`200 OK`
  ```json
  {"url": "https://..."}
...
- 失败响应：
  - `401` 未登录 → `{"error":"未登录"}`
  - `400` 缺少 user_id → `{"error":"缺少 user_id"}`
  - `404` 用户不存在 → `{"error":"用户不存在"}`
  - `404` 用户未设置头像 → `{"error":"用户未设置头像"}`
  - `404` 头像不存在 → `{"error":"头像不存在"}`
  - `400` 头像缺少存储信息 → `{"error":"头像缺少存储信息"}`
  - `410` 头像已过期 → `{"error":"头像已过期"}`
  - `503` 对象存储未就绪 → `{"error":"对象存储未就绪"}`
  - `503` 对象存储配置缺失 → `{"error":"对象存储配置缺失"}`
  - `500` 查询用户失败 → `{"error":"查询用户失败"}`
  - `500` 生成头像链接失败 → `{"error":"生成头像链接失败"}`

## 获取头像缩略图下载链接
- 方法与路径：`GET /api/users/:user_id/avatar/thumb/url`
- 认证：需要在请求头携带 `Authorization: Bearer <accessToken>`
- 说明：如果头像已生成缩略图，则返回缩略图链接；否则回退返回原头像链接。
- 成功响应：`200 OK`
  ```json
  {"url": "https://..."}
  ```
